...
function GestionDeCitas({
  ...
  const [datosCancelarCita, setDatosCancelarCita] = useState('')
  const [motivoCancelacion, setMotivoCancelacion] = useState('')
  const [errorCedula, setErrorCedula] = useState(false)
  const [incompleto, setIncompleto] = useState(false)
  const [dialogCancelarCita, setDialogCancelarCita] = useState(false)
  ...
  function cancelacionCitaPaciente(rowData) {
    setDialogCancelarCita(true)
    setDatosCancelarCita(rowData)
  }

  function acceptarCancelarCita() {
    cancelarCita({
      codCita: datosCancelarCita.co_cita_medica,
      motivo: motivoCancelacion,
      codUser: parseInt(co_usuario)
    }).then(({ cancelarCitaPaciente: { status, message, type } }) => {
      toast.current.show({
        severity: type,
        summary: 'Atención',
        detail: message,
  ...
  const headerCancelacion = (
    <motion.div
      ...
      className="bg-[#2a7e87] text-white w-[45%] redondeo-xl"
    >
      <h1>MOTIVOS DE LA CANCELACIÓN</h1>
    </motion.div>
  )
      ...
      >
        <div>
          ...
          >
            <Button
              label="Aceptar"
              disabled={!(motivoCancelacion.length > 12)}
              className="p-button-danger"
              onClick={() => acceptarCancelarCita()}
            />
          </motion.center>
        </div>
      </Dialog>
