...
import vistaRecepcion from 'graphql/recepcion'
...
function CrearCitas() {
  ...
  const [telefonoPersonal2, setTelefonoPersonal2] = useState('')
  /* Fin Solicitante */
  ...
  const [selectDiscapacidad, setSelectDiscapacidad] = useState(null)
  ...
  const { data: discapacidades } = useSWR(
    token ? [vistaRecepcion.GET_DISCAPACIDADES, {}, token] : null
  )
  ...
  const solicitar = (e) => {
    ...
    if (selectedTipoPac?.code === 2) {
      if (
        telefonoPersonal.length > 1 &&
        telefonoPersonal2.length > 1 &&
        ...
        solicitudFinal.discapacidad = selectDiscapacidad.id_discapacidad
          ? selectDiscapacidad.id_discapacidad
          : null
        ...
        solicitudFinal.telefono_personal = telefonoPersonal
        solicitudFinal.telefono_secundario = telefonoPersonal2
        ...
        toast.current.show({
          ...
          summary: 'Atención',
          detail:
            'El número telefónico primario no puede ser exactamente igual al segundario.',
          life: 3000
        })
        ...
        toast.current.show({
          ...
          summary: 'Atención',
          detail:
            'Los números telefónicos son requeridos para completar la solicitud.',
          life: 3000
        })
        setValidarTelefono(true)
      }
        ...
        consultaNacimientoEdadFun({
          ...
          ({ getEdadNacimientoFuncionario: { edad, fecha_nacimiento } }) => {
            if (fecha_nacimiento !== null) {
              solicitudFinal.fe_solicitud = new Date()
                .toISOString()
                .slice(0, 10)
              solicitudFinal.ced_solicitante = parseInt(cedula)
              solicitudFinal.nacionalidad_paciente = nacionalidad.code
              solicitudFinal.co_especialidad = selectedEspe.code
              solicitudFinal.co_tipo_paciente = selectedTipoPac.code
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        <div className="flex flex-row">
          <SelectButton
            ...
            className="nacionalidad basis-[70px]"
          />
          <div className="field">
            ...
            {errorCedula && (
              <small className="block text-red-600 text-center">
                Verifique la Cédula o Diríjase a Registrar Paciente.
              </small>
            )}
          </div>
        </div>
            ...
            <span className="p-float-label">
              <InputText
                ...
                className={`redondeo-lg w-[96%] ${
                  validarTelefono && telefonoPersonal.length < 1
                    ? 'border-red-400'
                    : ''
                }`}
                ...
                keyfilter="pint"
              />
              <label htmlFor="username">Teléfono del Paciente</label>
            </span>
          </div>
        )}
        {cedula?.length > 5 && !cargaNombreCompleto && (
          <div className="field">
            <span className="p-float-label">
              <InputText
                placeholder="Teléfono Personal"
                className={`redondeo-lg w-[96%] ${
                  validarTelefono && telefonoPersonal2.length < 1
                    ? 'border-red-400'
                    : ''
                }`}
                value={telefonoPersonal2}
                onChange={(e) => setTelefonoPersonal2(e.target.value)}
                maxLength={11}
                keyfilter="pint"
              />
              <label htmlFor="username">Teléfono Secundario</label>
            </span>
          </div>
        )}
        {cedula?.length > 5 && !cargaNombreCompleto && (
        ...
        {selectedTipoPac !== null && selectedTipoConsult?.code && (
          <center className="-mt-3">
            <label
              className="block"
              style={{ fontSize: '20px', fontWeight: '400' }}
        ...
        {selectedEspe?.code && selectedTipoPac && discapacidad?.code === 1 && (
          <div className="field">
            <span className="p-float-label">
              <Dropdown
                emptyMessage="No existen opciones disponibles"
                className="redondeo-lg w-[96%]"
                value={selectDiscapacidad}
                options={discapacidades?.getDiscapacidades}
                onChange={(e) => setSelectDiscapacidad(e.value)}
                optionLabel="nb_discapacidad"
              />
        ...
        {statusCita?.statusSolicitudCita.length >= 1 && cedula?.length > 5 && (
          <center id="selectedPaciente" className="col-span-4 -mt-4 h">
            <div
              style={{ fontSize: '30px', fontWeight: '600' }}
